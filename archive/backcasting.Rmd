...
source('R/package.R'); source('R/deconcolve.R'); source('R/simple_tdar.R'); source('R/package_tv.R');
...
parms <- data.frame(mean=rep(0, times=length(allcases[,1])),
                    sd=rep(0, times=length(allcases[,1])),
                    distribution=rep("lognorm", times=length(allcases[,1])))
...
for (row in 1:length(allcases[,1])) {
  if(allcases[row,"Date"] < as.Date("2020-01-09")){
    ...
    allcases[row,"mean"] = 0.2
    allcases[row,"sd"] = 0.45
  }
}
...
incubation_matrix = incubation_period_distribution_matrix_tv(outbreak_parms = allcases, rl = TRUE)
final_estimate = deconvolve_infection_curve_rl(symptom_onset_curve = allcases$cases,
                                               matrix = incubation_matrix,
                                               estimate = estimates)

allcases$estimate = round_infection_curve(allcases$cases, final_estimate)
