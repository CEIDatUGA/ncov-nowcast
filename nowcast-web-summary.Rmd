...
## Fit the exposed curve and forecast
...
database$exposed.fcast.mean <- exposed.forecast$mean %>% pad_na(timespan - na.tail, front = TRUE)
database$exposed.fcast.upper95 <- exposed.forecast$upper[,'95%'] %>% pad_na(timespan - na.tail, front = TRUE)
database$exposed.fcast.lower95 <- exposed.forecast$lower[,'95%'] %>% pad_na(timespan - na.tail, front = TRUE)
...
p_Exposed <- plotly::plot_ly(data = database, x = ~Date , y = ~cases, type = 'scatter',
  ...
  plotly::add_trace(y = ~exposed.fit, 
                    ...
                    line = list(color = col.fcast, width = 1, dash = 'dot')) %>% 
  plotly::add_trace(y = ~exposed.fcast.mean, 
                    name = 'exposed (forecast)', mode = 'lines',
                    line = list(color = col.fcast)) %>% 
  plotly::add_ribbons(ymin = ~exposed.fcast.lower95, ymax = ~exposed.fcast.upper95,
                      name = 'forecast 95% confidence', mode='lines',
