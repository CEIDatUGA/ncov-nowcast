...
Now the curve is set, giving six different estimations of the number of people beginning to show symptoms each day. Deconvolve each of the curves and use the average method. This time we exclude the Ridge regression, fourier filter, and frequency filter because they do not perform well in this truncated outbreak using this mean and standard deviation.
...
for (i in 1:6) {
  new=deconvolve_single_curve(final_curve[,i], parms = parms, skip = c("ridge","fourier","frequency"))
  infection_curve=cbind(infection_curve, make_df(new, skip = c("ridge","fourier","frequency")))
}

# First column contains the original curve, the subsequent columns contain an estimation of the infection curve
plot(infection_curve[1:53,1], type='l', ylim=c(0,3000))
...
The first ~53 indices are a legitimate estimate. After that it becomes less acurate because of the deconvolution. Any sharp downward trend is likely due to errors from the truncated outbreak (run during the durition of the outbreak instead at the end looking back). Estimations should be good up until then, but not all the way to the last row of the dataframe. 
